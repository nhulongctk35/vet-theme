...
export const getSortingLabel = (sorting) => {
  ...
  const label = sorting.order && (sorting.order === "desc" ? descLabel : ascLabel)
...
export const simplifyString = (phrase) =>
  (typeof phrase === "string" && phrase.replace(/[^a-zA-Z0-9]+/g, "-")) || phrase

export const formatDate = (date, format = `DD-MM-YYYY HH:mm:ss`) => dayjs(date).format(format)

export const getSearchPageUrl = (searchTerm) => `${PAGE_SEARCH}?query=${encodeURIComponent(searchTerm)}`
